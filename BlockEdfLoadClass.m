classdef BlockEdfLoadClass
    ...
    properties (Access = public)
        ...
        SWAP_MIN_MAX = 1;          % Swap if digMin > difMax; phyMin > phyMax
    end
     %------------------------------------------------ Dependent Properties
            ...
            if and(~isempty(obj.signalHeader),obj.numCompToLoad>1)
                ...
                for s = 1:obj.header.num_signals
                   ...
                   if sum(error)< length(reserve_2{s})
                       errMsg = sprintf...
                           ('Signal Header: Non ASCII character in reserve_2 field, signal %.0f',...
                                s); 
                       errorMSG(end+1) = {errMsg};
                       errorSeverity(end+1) = edfDeviation;
                   end
    ...
    methods
       ...
       function value = get.totNumDeviations(obj)
            ...
            deviationByType = obj.deviationByType;
            
            % Check if there are errors
            if deviationByType(1) == 1 
            	value = 0;
            else
                value = sum(deviationByType);
            end       
       end
       %--------------------------------------------------- deviationByType
